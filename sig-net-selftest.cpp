...
void TestCryptoModule(TestSuiteResults& results) {
    ...
    // Test 8: CRC-16 Calculation (test vector for K0 validation)
    // Using test K0 from project notes
    {
        uint8_t test_k0_bytes[32];
        // Parse hex: A7B3C8D2E1F45926384A5B6C7D8E9F0121324354657687980A1B2C3D4E5F6071
        const uint8_t expected_k0[32] = {
            0xA7, 0xB3, 0xC8, 0xD2, 0xE1, 0xF4, 0x59, 0x26,
            0x38, 0x4A, 0x5B, 0x6C, 0x7D, 0x8E, 0x9F, 0x01,
            0x21, 0x32, 0x43, 0x54, 0x65, 0x76, 0x87, 0x98,
            0x0A, 0x1B, 0x2C, 0x3D, 0x4E, 0x5F, 0x60, 0x71
        };
        memcpy(test_k0_bytes, expected_k0, 32);
        
        uint16_t crc = Crypto::CalculateK0_CRC16(test_k0_bytes, 32);
        uint16_t expected_crc = 0xEEB2;  // From project notes
        
        bool passed = (crc == expected_crc);
        char msg[256];
        snprintf(msg, sizeof(msg), "Expected 0x%04X, got 0x%04X", expected_crc, crc);
        AddTestResult(results, "Crypto: CRC-16 Known Vector",
                      passed, passed ? "" : msg);
    }
}
